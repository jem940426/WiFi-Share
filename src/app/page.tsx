...
export default function Home() {
  ...
  const isPremiumBlocked = activeTemplateInfo.isPremium && (isTemplateLocked || !isInputComplete);
  const isPreviewBlocked = isSsidEmpty || isPremiumBlocked;
        ...
        <div className="w-full flex flex-col lg:flex-row items-center lg:items-start justify-center gap-12 lg:gap-20">
          ...
          <div className="w-full lg:w-auto flex flex-col items-center lg:items-start pl-0 lg:pl-4 mt-8 lg:mt-0 relative">
            <div className="relative group perspective-1000">
              <div className={`absolute -inset-1 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-[2.5rem] blur transition duration-1000 ${isPreviewBlocked ? 'opacity-20' : 'opacity-25 group-hover:opacity-50 group-hover:duration-200'}`}></div>
              <div className={`relative transform transition-all duration-500 ${isPreviewBlocked ? 'select-none pointer-events-none' : ''}`}>
                <WiFiPreview ref={previewRef} templateId={templateId} wifiString={displayWifiString} ssid={displaySsid} isLocked={isPreviewBlocked} />
              </div>
              
              {/* Empty state overlay (priority over lock overlay) */}
              {isSsidEmpty && !isTemplateLocked && !activeTemplateInfo.isPremium && (
                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center">
                  <div className="bg-black/60 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/20 shadow-2xl">
                    <p className="text-white font-bold tracking-wide">네트워크 이름을<br/>입력해주세요</p>
                  </div>
                </div>
              )}

              {/* Locked / Premium Blocked overlay (New Watermark UX) */}
              {isPremiumBlocked && (
